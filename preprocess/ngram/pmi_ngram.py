...
parser.add_argument("--dataset", type=str, default="path of \mimic_cxr", help="the name of dataset")
parser.add_argument("--output_dir", type=str,default="path of mimic_cxr_ngram.txt", help="the output path")
parser.add_argument("--ngram", type=int, default=4, help="n")
parser.add_argument("--min_count", type=int, default=3, help="min_count")
parser.add_argument("--min_pmi", type=int, default=1, help="min_pmi")
parser.add_argument(
    "--ngram_freq_threshold", type=int, default=50, help="ngram_freq_threshold"
)
parser.add_argument(
...
if config.dataset == "mimic_cxr" or config.dataset == "path of \mimic_cxr":
...
with open("path of mimic_grams.json", "r", encoding="utf-8") as f:
    ff = json.load(f)
...
for report in tqdm(f_read, desc="Loading"):
    ...
    for sen1 in sentence_list:
        ...
        for i in range(len(sen)):
            if sen[i] in ngrams:
                gram.add(ngrams.index(sen[i]))
                print(sen[i])

            if i < len(sen)-1 and (sen[i]+" "+sen[i+1]) in ngrams:
                gram.add(ngrams.index(sen[i]+" "+sen[i+1]))
                print(sen[i]+" "+sen[i+1])
            if i < len(sen)-2 and (sen[i]+" "+sen[i+1]+" "+sen[i+2]) in ngrams:
                gram.add(ngrams.index(sen[i]+" "+sen[i+1]+" "+sen[i+2]))
                print(sen[i]+" "+sen[i+1]+" "+sen[i+2])
            if i < len(sen)-3 and (sen[i]+" "+sen[i+1]+" "+sen[i+2]+" "+sen[i+3]) in ngrams:
                gram.add(ngrams.index(sen[i]+" "+sen[i+1]+" "+sen[i+2]+" "+sen[i+3]))
                print(sen[i]+" "+sen[i+1]+" "+sen[i+2]+" "+sen[i+3])
    dct[id] = list(gram)
    ...
    def text_filter(self, sentence):
        ...
        for i, w in enumerate(sentence):
            if re.match("[^\u0600-\u06FF\u0750-\u077F\u4e00-\u9fa50-9a-zA-Z]+", w):
                #正则表达式“^\d{17}(\d|X)$”来匹配身份证号码。其中，^表示字符串开头，\d表示数字，{17}表示匹配前面的\d重复17次，
                # (\d|X)表示匹配一个数字或者X，$表示字符串结尾。如果匹配成功，返回True；否则，返回False。
                if i > index:
