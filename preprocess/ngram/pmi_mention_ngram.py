...
parser.add_argument("--chexbert_dir", type=str,default='path of mimic_label.csv', help="the output path")
parser.add_argument("--output_dir", type=str,default="path of mimic_cxr_pmi.json", help="the output path")
parser.add_argument(
    "--pmi_threshold", type=float, default=0.0, help="the output path"
)
parser.add_argument("--pycx_dir", type=str, default="path of mimic_cxr_pycx.json",help="the output path")
...
for header in headers:
    topics[header+" "+"True"] = header+"_True"
    topics[header+" "+"False"] = header+"_False"
with open("../../data/iu_xray_topics.json", "w", encoding="utf-8") as f:
...
for xy in p_xy:
    y, x = xy.split("@")
    try:
        pmi_xy = math.log(p_xy[xy] ** k / (p_x[x] ), 2) #* p_y[y]
