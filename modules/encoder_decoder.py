...
from modules.TopicGuidedAlignment import Query_model, Sparsemax
...
class Transformer(nn.Module):
    def __init__(self, node_encoder,g_encoder,gram_encoder,encoder, decoder, src_embed, tgt_embed, node_embed):
    ...
    def forward(self,text_feature,txt_mean,src, tgt, src_mask, tgt_mask,text_sigma,topic_sigma,topic_features,hidden_states):

        node_embeddings = self.get_node_embedding()
        gram_embeddings = self.get_gram_embedding()
        ind,selected_feature = self.topic_infer(text_feature, text_sigma, topic_sigma, topic_features)
        b_num = tgt.size(0)
    ...
    def get_adjusted_topic_logits(self,feature,text_sigma,topic_sigma,topic_features):
        ...
        for i in range(512):
            det = det + 0.5 * torch.log(topic_sigma[:, i] ** 2 + 1e-12)
        const = -(det)
    ...
    def get_topic_logits(self,feature,text_sigma,topic_sigma,topic_features):
    ...
    def topic_infer(self,text_feature, text_sigma, topic_sigma, topic_features):

        topic_features = topic_features.squeeze(dim=0)
        prob = self.get_topic_logits(text_feature, text_sigma, topic_sigma,  topic_features)
...
class TextEncoder(nn.Module):
    ...
    def forward(self,x,bert_mask,mask):
        x = self.seqembed(x)
        text_mean = torch.sum((x * mask), dim=1)
        x = torch.cat([text_mean.unsqueeze(dim=1), x], dim=1)
        for layer in self.layers:
            x = layer(x, bert_mask)
        return self.norm(x)
...
class GramPlan(nn.Module):
    def __init__(self,g_cross_attn, feed_forward, d_model, dropout):
        super(GramPlan, self).__init__()
        self.g_cross_attn = g_cross_attn
        self.feed_forward = feed_forward
    ...
    def graph_cross_attn(self, x, m):
        b, n, d = x.size()
        all_q = torch.empty((0, n, d)).to(x.device)
        for i in range(b):
            result = self.g_cross_attn(x[i].unsqueeze(dim=0), m[i].unsqueeze(dim=0), m[i].unsqueeze(dim=0))
            all_q = torch.cat([all_q, result], dim=0)

        return all_q

    def forward(self,x,grams,gram_masks):
        x1 = self.sublayer[0](x, lambda x: self.g_cross_attn(x, grams,grams,gram_masks), 0)
        x1 = self.linear1(x1)
        x2 = self.linear2(x)
        xx = x1+x2
        x = self.linear3(torch.tanh(xx))+x
        return x

class Decoder(nn.Module):
    def __init__(self, layer,gramplan, N):
        super(Decoder, self).__init__()
        self.layers = clones(layer, N)
        self.norm = LayerNorm(layer.d_model)
        self.gramplan = gramplan
...
class AlignTopic(nn.Module):
    def __init__(self,  sd_num, sd_dim,
                 att_func_type, pool_type, sd_temperature):
        super().__init__()



        self.img_query_model = Query_model(ft_dim=sd_dim, sd_dim=sd_dim, temperature=sd_temperature,
                                           att_func_type=att_func_type, pool_type=pool_type)
        self.txt_query_model = Query_model(ft_dim=sd_dim, sd_dim=sd_dim, temperature=sd_temperature,
                                           att_func_type=att_func_type, pool_type=pool_type)

        self.logit_scale = nn.Parameter(torch.ones([1]))
        self.logit_scale_sd = nn.Parameter(torch.ones([1]))
        nn.init.constant_(self.logit_scale, 1)
        nn.init.constant_(self.logit_scale_sd, 1 )
    ...
    def forward(self, image_mean,text_mean,image_feature, text_feature,pad_mask,topic_features,text_sigma,image_sigma):
        n = image_mean.size(0)
        det = 1
        for i in range(512):
            det = det*(text_sigma[i]/(image_sigma[i]+1e-8))

        residual = text_mean - image_mean
        divergence_loss = 1/2* (torch.sum(torch.diagonal((residual/((0.1*image_sigma.unsqueeze(dim=0))**2) @ residual.t()), dim1=0, dim2=1)) +
                                          +n*(torch.sum((text_sigma/image_sigma)**2)-torch.log((det)**2)) )




        return divergence_loss/n
...
class EncoderDecoder(AttModel):
    ...
    def make_textual_encoder(self):
        ...
        t_encoder = TextualLayer(dim, c(attn), c(ff), self.dropout)
    ...
    def make_model(self, tgt_vocab):
        ...
        attn = MultiHeadedAttention(self.num_heads, self.d_model)
        ff = PositionwiseFeedForward(self.d_model, self.d_ff, self.dropout)
        gp = GramPlan(c(attn), c(ff), self.d_model, self.dropout)
    ...
    def __init__(self, args, tokenizer):
        ...
        self.Align = AlignTopic(sd_num= 28, sd_dim=512,
                 att_func_type='sparsemax', pool_type='max', sd_temperature=10)
        ...
        self.ff3 = PositionwiseFeedForward(self.d_model, self.d_ff, self.dropout)
        ...
        self.topic_features = torch.empty(0)
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks,topic_features,gram_embeddings,node_embeddings):
        ...
        ind, selected_feature = self.model.topic_infer( image_mean1, self.image_sigma,self.topic_sigma, topic_features)
    ...
    def TopicCL(self,img_mean,text_mean,img_feature,text_feature,mask,topicfeatures,text_sigma,image_sigma ):
        divergence = self.Align(img_mean,text_mean,img_feature,text_feature,mask[:,1:],topicfeatures,text_sigma,image_sigma)
        return divergence
    ...
    def CE(self,labels,logits):
        ...
        label = labels
        logit = torch.log(logits  + 2e-10)


        for i in range(b):
            loss = -label[i]  @ logit[i].t()
            ce_loss += loss
    ...
    def Draw_infer(self,mean,dev):
        b_num, dim_num = mean.size()
        k = 5
        rand = torch.normal(0, 1, (1, k, dim_num)).to(device)
        rand = torch.mean(rand, 1)
        mean = 0.1 * rand * dev + mean
        return mean
    ...
    def _forward(self, report_embed,fc_feats, att_feats, seq, labels,att_masks=None):
        ...
        text_features = self.text_encoder(seq[:,1:], bert_mask,mask[:,1:,:])
        ...
        self.topic_features = self.get_topic_feature(report_embed)


        #get probabilities
        img_topic_logits = self.model.get_adjusted_topic_logits(image_mean1, self.image_sigma, self.topic_sigma,
                                                        self.topic_features.detach())
        text_topic_logits = self.model.get_adjusted_topic_logits(text_mean1, self.text_sigma, self.topic_sigma,
                                                         self.topic_features)

        #get classification loss
        clf_loss = self.reverse_CE(labels, img_topic_logits) + self.reverse_CE(labels, text_topic_logits)

        #get kl divergence
        divergence = self.TopicCL(image_mean,text_mean.detach(),image_features, text_features, mask, self.topic_features,
                                            self.text_sigma, self.image_sigma)

        text_mean2 = self.Draw_infer(text_mean, self.text_sigma)
        txt_mean = self.Linear2(text_mean2)
        out= self.model(text_mean2,txt_mean,att_feats, seq, att_masks, seq_mask,self.text_sigma,self.topic_sigma,self.topic_features,image_features)
        ...
        return outputs,clf_loss,divergence
