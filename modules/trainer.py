...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch,report_embed):
        ...
        for batch_idx, (chxt_label, images, reports_ids, reports_masks) in enumerate(self.train_dataloader):
            ...
            output,clfloss,div = self.model(report_embed,images, chxt_label,topic_features,node_embeddings,reports_ids,mode='train')

            #loss = torch.tensor(0,dtype = torch.float32).to(alignloss.device)
        ...
        with torch.no_grad():
            ...
            node_embeddings.append(node)
            node_embeddings.append(self.model.encoder_decoder.model.get_gram_embedding())
            ...
            for batch_idx, (chxt_label, images, reports_ids, reports_masks) in enumerate(self.val_dataloader):
                ...
                output,_,_ = self.model(report_embed,images, chxt_label,topic_features,node_embeddings,mode='sample')

                reports = self.model.tokenizer.decode_batch(output.cpu().numpy())
                ground_truths = self.model.tokenizer.decode_batch(reports_ids[:, 1:].cpu().numpy())
                val_res.extend(reports)
                val_gts.extend(ground_truths)
                print('generated report:', reports[0])
